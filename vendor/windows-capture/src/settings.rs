...
pub struct Settings<Flags, T: TryInto<GraphicsCaptureItemType>> {
    ...
    /// Number of WGC-owned surfaces. JustIn Replay keeps this deliberately small because encoder
    /// input is detached to separately owned GPU textures before the callback returns.
    pub(crate) frame_pool_buffer_count: i32,
}
...
impl<Flags, T: TryInto<GraphicsCaptureItemType>> Settings<Flags, T> {
    ...
    pub const fn new(
        ...
        Self {
            ...
            color_format,
            flags,
            frame_pool_buffer_count: 2,
        }
    }

    /// Sets the number of WGC frame-pool surfaces. Values are clamped to the supported test range
    /// of 1 through 3 so callers cannot accidentally mask a retained-surface problem with a large
    /// pool.
    #[inline]
    #[must_use]
    pub fn frame_pool_buffer_count(mut self, count: u32) -> Self {
        self.frame_pool_buffer_count = count.clamp(1, 3) as i32;
        self
    }
    ...
    /// Returns the configured WGC frame-pool surface count.
    #[inline]
    #[must_use]
    pub const fn frame_pool_buffers(&self) -> i32 {
        self.frame_pool_buffer_count
    }
}
