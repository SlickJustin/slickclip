...
impl GraphicsCaptureApi {
    ...
    pub fn new<T: GraphicsCaptureApiHandler<Error = E> + Send + 'static, E: Send + Sync + 'static>(
        ...
        // Create frame pool
        // A free-threaded pool delivers frames without depending on a DispatcherQueue. The pool is
        // intentionally small: encoder input must detach from these surfaces before the callback
        // returns, so increasing this count is not the primary backpressure mechanism.
        ...
        >::new({
            ...
            let recreate_buffer_count = frame_pool_buffer_count;
