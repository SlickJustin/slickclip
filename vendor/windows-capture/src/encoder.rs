...
#[derive(Default)]
struct EncoderTelemetryInner {
    source_frames_received: AtomicU64,
    owned_frame_copies: AtomicU64,
    gpu_copy_total_ns: AtomicU64,
    gpu_copy_worst_ns: AtomicU64,
...
/// Shared, lock-free telemetry for the bounded detached-frame encoder queue.
#[derive(Clone)]
pub struct EncoderTelemetry {
    inner: Arc<EncoderTelemetryInner>,
    queue_capacity: usize,
}

/// A point-in-time view of encoder queue and GPU detachment activity.
#[derive(Clone, Copy, Debug, Default)]
pub struct EncoderTelemetrySnapshot {
    /// WGC-backed frames submitted to the detached DirectX path.
    pub source_frames_received: u64,
    /// Encoder-owned GPU textures populated by a GPU copy.
    pub owned_frame_copies: u64,
    /// Average CPU time to allocate the owned texture and submit the GPU copy.
    pub gpu_copy_average_ms: Option<f64>,
    /// Worst CPU time to allocate the owned texture and submit the GPU copy.
    pub gpu_copy_worst_ms: Option<f64>,
    /// Current number of owned surfaces waiting for MediaStreamSource.
    pub queue_depth: u64,
    ...
    /// Frames deliberately dropped rather than blocking a realtime producer.
...
impl EncoderTelemetry {
    fn new(queue_capacity: usize) -> Self {
        Self {
            inner: Arc::new(EncoderTelemetryInner::default()),
            queue_capacity,
        }
    }

    #[must_use]
    /// Captures current counters without blocking the encoder worker.
    pub fn snapshot(&self) -> EncoderTelemetrySnapshot {
        let copies = self.inner.owned_frame_copies.load(Ordering::Relaxed);
        let total_ns = self.inner.gpu_copy_total_ns.load(Ordering::Relaxed);
        let worst_ns = self.inner.gpu_copy_worst_ns.load(Ordering::Relaxed);
        EncoderTelemetrySnapshot {
            source_frames_received: self.inner.source_frames_received.load(Ordering::Relaxed),
            owned_frame_copies: copies,
            gpu_copy_average_ms: (copies > 0).then(|| total_ns as f64 / copies as f64 / 1_000_000.0),
            gpu_copy_worst_ms: (copies > 0).then_some(worst_ns as f64 / 1_000_000.0),
            queue_depth: self.inner.queue_depth.load(Ordering::Relaxed),
            maximum_queue_depth: self.inner.maximum_queue_depth.load(Ordering::Relaxed),
            queue_capacity: self.queue_capacity,
            queue_full_events: self.inner.queue_full_events.load(Ordering::Relaxed),
            deliberately_dropped_frames: self.inner.deliberately_dropped_frames.load(Ordering::Relaxed),
        }
    }

    fn record_copy(&self, duration: Duration) {
        let nanos = duration.as_nanos().min(u128::from(u64::MAX)) as u64;
        self.inner.owned_frame_copies.fetch_add(1, Ordering::Relaxed);
        self.inner.gpu_copy_total_ns.fetch_add(nanos, Ordering::Relaxed);
        self.inner.gpu_copy_worst_ns.fetch_max(nanos, Ordering::Relaxed);
    }

    fn record_full_drop(&self) {
        self.inner.queue_full_events.fetch_add(1, Ordering::Relaxed);
        self.inner.deliberately_dropped_frames.fetch_add(1, Ordering::Relaxed);
    }
}

/// Result of one nonblocking detached-frame submission.
#[derive(Clone, Copy, Debug)]
pub struct FrameSendResult {
    /// Whether the owned surface entered the encoder queue.
    pub queued: bool,
    /// CPU duration of owned-texture allocation and GPU-copy submission, if performed.
    pub gpu_copy_duration: Option<Duration>,
    /// Queue depth observed when the submission completed.
    pub queue_depth: u64,
}
...
///
/// - For [`VideoEncoderSource::DirectX`], both the encoder-owned texture and its WinRT surface are
///   retained until a `MediaStreamSample` has taken its own surface reference.
...
/// An encoder-owned D3D11 texture and its WinRT surface projection.
...
impl VideoSettingsBuilder {
    ...
    /// Sets the maximum number of encoder-owned GPU surfaces waiting for MediaStreamSource.
    /// A full queue drops new frames instead of blocking the capture callback.
    pub const fn frame_queue_capacity(mut self, capacity: usize) -> Self {
        self.frame_queue_capacity = if capacity == 0 { 1 } else { capacity };
        self
    }
...
impl VideoEncoder {
            ...
            if is_audio {
                ...
                ThreadPool::RunWithPriorityAndOptionsAsync(
                    &WorkItemHandler::new(move |_| {
                        ...
                        match value {
                            Ok(Some((source, timestamp))) => {
                                telemetry.inner.queue_depth.fetch_sub(1, Ordering::Relaxed);
                                let sample = match source {
                                    VideoEncoderSource::DirectX(surface) => {
                                        MediaStreamSample::CreateFromDirect3D11Surface(&surface.surface.0, timestamp)?
                                    }
    ...
    pub fn new<P: AsRef<Path>>(
        ...
        let frame_queue_capacity = video_settings.frame_queue_capacity;
        ...
        let (frame_sender, frame_receiver_raw) =
            mpsc::sync_channel::<Option<(VideoEncoderSource, TimeSpan)>>(frame_queue_capacity);
        let (audio_sender, audio_receiver_raw) = mpsc::channel::<Option<(AudioEncoderSource, TimeSpan)>>();
        let telemetry = EncoderTelemetry::new(frame_queue_capacity);
    ...
    fn detach_surface(&self, frame: &Frame) -> Result<OwnedSurface, VideoEncoderError> {
        ...
        if frame.width() == self.target_width && frame.height() == self.target_height {
            // GPU-only detachment. The immediate context orders this copy before later encoder use,
            // and D3D11 retains command resources until the asynchronous GPU operation completes.
            unsafe {
                context.CopyResource(&owned.texture.0, frame.as_raw_texture());
            }
        } else {
            let mut render_target = None;
            unsafe {
                frame.device().CreateRenderTargetView(
                    &owned.texture.0,
                    None,
    ...
    fn queue_source(
        ...
        timestamp: TimeSpan,
        gpu_copy_duration: Option<Duration>,
    ) -> Result<FrameSendResult, VideoEncoderError> {
        let pending_depth = self.telemetry.inner.queue_depth.fetch_add(1, Ordering::Relaxed) + 1;
        let message = Some((source, timestamp));
        match self.frame_sender.try_send(message) {
            Ok(()) => {
                self.telemetry
                    .inner
                    .maximum_queue_depth
                    .fetch_max(pending_depth, Ordering::Relaxed);
                Ok(FrameSendResult {
                    queued: true,
                    gpu_copy_duration,
            ...
            Err(mpsc::TrySendError::Full(_)) => {
                self.telemetry.inner.queue_depth.fetch_sub(1, Ordering::Relaxed);
            ...
            Err(mpsc::TrySendError::Disconnected(message)) => {
                self.telemetry.inner.queue_depth.fetch_sub(1, Ordering::Relaxed);
                Err(VideoEncoderError::FrameSendError(mpsc::SendError(message)))
            }
        }
    }

    /// Copies a WGC frame into an encoder-owned GPU texture and queues only that owned surface.
    /// The bounded queue is nonblocking: when full, the new frame is deliberately dropped.
    #[inline]
    pub fn send_frame_with_result(&mut self, frame: &Frame) -> Result<FrameSendResult, VideoEncoderError> {
        ...
        if self.telemetry.inner.queue_depth.load(Ordering::Relaxed) >= self.telemetry.queue_capacity as u64 {
            self.telemetry.record_full_drop();
        ...
        let surface = self.detach_surface(frame)?;
        let copy_duration = copy_started.elapsed();
        self.telemetry.record_copy(copy_duration);
        let result = self.queue_source(VideoEncoderSource::DirectX(surface), timestamp, Some(copy_duration))?;
    ...
    /// Sends a video frame through the detached, bounded GPU queue.
    #[inline]
    pub fn send_frame(&mut self, frame: &Frame) -> Result<(), VideoEncoderError> {
        self.send_frame_with_result(frame).map(|_| ())
    }
    ...
    pub fn send_frame_with_audio(&mut self, frame: &mut Frame, audio_buffer: &[u8]) -> Result<(), VideoEncoderError> {
        ...
        let _ = self.send_frame_with_result(frame)?;
    ...
    pub fn send_frame_buffer(&mut self, buffer: &[u8], timestamp: i64) -> Result<(), VideoEncoderError> {
        ...
        let _ = self.queue_source(VideoEncoderSource::Buffer(buffer.to_vec()), timestamp, None)?;
    ...
    /// Returns a cloneable telemetry handle that remains valid through finalization.
    #[inline]
    #[must_use]
    pub fn telemetry(&self) -> EncoderTelemetry {
